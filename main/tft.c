    ...
    while (1) {
        if(xQueueReceive(xQueueImu, &motion6, portMAX_DELAY)) {
            if (motion6.sender == SENDER_IMU) {
                lcdDrawCircle(&dev, xpos, ypos, 10, rgb565(64, 64, 64)); // paint it black
            ...
            if (motion6.sender == SENDER_BUTTON_A) {
                App_action[g_appindex]();
                //spi_master_write_command(&dev, 0x11);
                //AXP192_ScreenBreath(9);
            }
            if (motion6.sender == SENDER_BUTTON_B) {
                ++g_appindex;
                if (g_appindex == APP_COUNT) g_appindex = 0;
                App_init[g_appindex]();
                //lcdFillScreen(&dev, BLACK);
